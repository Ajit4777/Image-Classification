...
def get_label_name(path):
    ...
    for dir  in dirs:
        print(dir)
    # print(np.identity(len(dirs),dtype=int))
    return np.identity(len(dirs)), dirs
...
model = tflearn.DNN(create_model(), tensorboard_dir ='log')
...
# 	# DIY One hot encoder 
# 	if word_label == 'RB': return [1, 0] 
# 	elif word_label == 'HUL': return [0, 1] 
...
#     ret, frame = cap.read()
#     width = 50#int(img.shape[1] * scale_percent / 100)
#     height = 50#int(img.shape[0] * scale_percent / 100)
#     dim = (width, height)
...
#     if np.argmax(model_out) == 1: str_label ='HUL'
#     else: str_label ='RB'
#     print(str_label)
